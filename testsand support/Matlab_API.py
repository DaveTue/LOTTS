...
class Simulink:
    ...
    def initialize_model2(self,ini_val={})-> None:
        """_summary_
    ...
    def initialize_model(self,values = []):
    ...
    def simulate(self):
        ...
        while(self.eng.get_param(self.modelName,'SimulationStatus') != ('stopped' or 'terminating')):
            ...
            print(self.get_output('input'))
            print(self.get_output('output'))
            #self.yHist,self.tHist = self.getHistory()
            time.sleep(1)
...
#model = Simulink(modelName="API_testmodel", directory ="C:\\Users\\20181234\\PersonalDave\\dsl-orchestration")
#Establishes a Connection
#model.connectToMatlab()
#model.Load_model()
...
#test for creating a s_function block with connection in simulink
#function_name = 's-interface'
#block_name = 'interface'

#sim_model = Simulink(modelName = 'Gain_model_withInterface.slx', directory = os.getcwd())
'''
sim_model.connectToMatlab()
sim_model.Load_model() #model needs to be open to work (this method load and open the model)
#sim_model.eng.eval("model_name = '{}'".format(sim_model.modelName),nargout=0) #store name of the block in matlab
#self.eng.eval("load_system(model)",nargout=0)
clean_name_model = sim_model.modelName.rsplit('.slx')[0]
name_param = clean_name_model + '/' + block_name
sim_model.eng.eval("func_name = '{}'".format(function_name),nargout=0) #store name of the function in matlab
sim_model.eng.eval("model_name = '{}'".format(clean_name_model),nargout=0) #store name of the block in matlab
sim_model.eng.eval("name_param = '{}'".format(name_param),nargout=0) #store name of the block in matlab

sim_model.eng.eval("add_block('simulink/User-Defined Functions/Level-2 MATLAB S-Function',name_param,'FunctionName',func_name)",nargout=0)
print('model ' + clean_name_model + ' has been changed with a new block named ' + block_name)


sim_model.eng.eval("save_system(model_name)",nargout =0)
sim_model.disconnect
'''
#directory = os.getcwd()
#model = Simulink(modelName = 'Gain_model', directory = directory)
#print(directory)
#model.interface_gen()